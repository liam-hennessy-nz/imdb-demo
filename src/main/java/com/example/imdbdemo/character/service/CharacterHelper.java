...
public class CharacterHelper {
	...
	public static Predicate toPredicate(MultiValueMap<String, String> params) {
		...
		for (PageHelper.ParsedFilter filter : filters) {
			...
			switch (field) {
				case "id" -> applyNumberOperator(bb, CHARACTER.id, op, val, Long::parseLong);
				case "ordering" -> applyNumberOperator(bb, CHARACTER.ordering, op, val, Short::parseShort);
				case "name" -> applyStringOperator(bb, CHARACTER.name, op, val);
