...
public class RawTitleBasicService {
	...
	private Page<RawTitleBasicDTO> search(Predicate predicate, OrderSpecifier<?>[] orderSpecifiers, Pageable pageable) {
		...
		Long total = queryFactory.select(TITLE_BASIC.count()).from(TITLE_BASIC).where(predicate).fetchOne();
