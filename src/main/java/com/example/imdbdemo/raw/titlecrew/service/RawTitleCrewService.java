...
public class RawTitleCrewService {
	...
	private Page<RawTitleCrewDTO> search(Predicate predicate, OrderSpecifier<?>[] orderSpecifiers, Pageable pageable) {
		...
		Long total = queryFactory.select(TITLE_CREW.count()).from(TITLE_CREW).where(predicate).fetchOne();
	...
	public RawTitleCrewDTO findById(Long id) {
		...
		return rawTitleCrew
			.map(rawTitleCrewMapper::mapToDto)
