...
public class RawTitleAkaService {
	...
	private Page<RawTitleAkaDTO> search(Predicate predicate, OrderSpecifier<?>[] orderSpecifiers, Pageable pageable) {
		...
		Long total = queryFactory.select(TITLE_AKA.count()).from(TITLE_AKA).where(predicate).fetchOne();
